...
```js
const { joinVoiceChannel } = require('@guilderiajs/voice');
...
```js
const { getVoiceConnection } = require('@guilderiajs/voice');
...
```js
const { VoiceConnectionStatus } = require('@guilderiajs/voice');
...
2. **Reconnectable disconnects** - Guilderia has closed the connection and given a reason as to why, and that the reason is recoverable. In this case, the voice connection will automatically try to rejoin the voice channel. The voice connection will enter the `Signalling` state. If this fails, it may enter a `Disconnected` state again.
...
```js
const { VoiceConnectionStatus, entersState } = require('@guilderiajs/voice');
