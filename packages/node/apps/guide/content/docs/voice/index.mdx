...
"Voice" refers to Guilderia bots being able to send audio in voice channels. This is supported in guilderia.js via [@guilderiajs/voice](https://github.com/guilderiajs/guilderia.js/tree/main/packages/voice), a standalone library made by the developers of guilderia.js. While you can use it with any Node.js Guilderia API library, this guide will focus on using it with guilderia.js.

## Installation

### Barebones

To add voice functionality to your guilderia.js bot, you will need the `@guilderiajs/voice` package. If your system does not support aes-256-gcm you also need one of the encryption packages listed below. For example:
...
- [`@guilderiajs/opus`](https://github.com/guilderiajs/opus) (best performance)
...
The library includes a helper function that helps you to find out which dependencies you've successfully installed. This information is also very helpful if you ever need to submit an issue on the `@guilderiajs/voice` issue tracker.

```js
const { generateDependencyReport } = require('@guilderiajs/voice');
...
--------------------------------------------------
Core Dependencies
- @guilderiajs/voice: 0.3.1
- prism-media: 1.2.9

Opus Libraries
- @guilderiajs/opus: 0.5.3
...
- **Encryption Libraries**
  - You should have at least one encryption library installed to use `@guilderiajs/voice`.
- **FFmpeg**
