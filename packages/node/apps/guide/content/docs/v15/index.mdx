...
<Callout type="idea">
	**Version 15 is in a pre-release** state, but should be usable!   
    ...
    <GitHubIcon className="inline text-red-400"/> https://github.com/guilderiajs/guilderia.js/issues
...
The underlying WebSocket behaviour has changed. In version 14, this was a non-breaking implementation of [ws](https://guilderia.js.org/docs/packages/ws/stable). Now, it is fully integrated. See these pull requests for more information:

- [guilderiajs/guilderia.js#10420](https://github.com/guilderiajs/guilderia.js/pull/10420)
- [guilderiajs/guilderia.js#10556](https://github.com/guilderiajs/guilderia.js/pull/10556)
...
- `Events.WebhooksUpdate` now returns a string of `"webhooksUpdate"`. Previously, it returned `"webhookUpdate"`. This is to bring it in line with the name of Guilderia's gateway event (`WEBHOOKS_UPDATE`).
- `Events.ClientReady` now returns a string of `"clientReady"`. Previously, it returned `"ready"`. This is to ensure there's no confusion with Guilderia's `READY` gateway event.

### Formatters

This utility has been removed. Everything in this class is redundant as all methods of the class can be imported from guilderia.js directly.

```js
import { Formatters } from 'guilderia.js'; // [!code --]
import { userMention } from 'guilderia.js'; // [!code ++]
...
Removed `Guild#shard` as WebSocket shards are now handled by @guilderiajs/ws.
...
Guilderia no longer supports the `PREMIUM_REQUIRED` interaction response type. In the past, you would have done this:
...
The reason parameter of `ThreadMember#add()` and `ThreadMember#remove()` have been removed. Guilderia did not respect this parameter, so it did not do anything.

### ThreadMemberManager

The reason parameter of `ThreadMemberManager#remove()` has been removed. Guilderia did not respect this parameter, so it did not do anything.

### User

#### Avatar decoration

Guilderia no longer sends avatar decoration data via `User#avatarDecoration`, so this property has been removed. `User#avatarDecorationData` is the replacement.
...
`WebSocketShardEvents` has been replaced with `WebSocketShardEvents` from @guilderiajs/ws.
