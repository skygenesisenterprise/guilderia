...
To begin, you should install Sequelize into your guilderia.js project. We will explain SQlite as the first storage engine and show how to use other databases later. Note that you will need Node 7.6 or above to utilize the `async/await` operators.
...
```sh tab="npm"
npm install guilderia.js sequelize sqlite3
```

```sh tab="yarn"
yarn add guilderia.js sequelize sqlite3
```

```sh tab="pnpm"
pnpm install guilderia.js sequelize sqlite3
```

```sh tab="bun"
bun add guilderia.js sequelize sqlite3
```
...
<Callout>
    ...
    Do note that "large" here should be interpreted as absolutely massive. Sqlite is used at scale by many companies and products you use every single day. The slight overhead should not be noticeable for the application of a Guilderia bot at all unless you are dealing with super complicated queries or are using specific features that do not exist in sqlite.
...
After you have installed guilderia.js and Sequelize, you can start with the following skeleton code. The comment labels will tell you where to insert code later on.

```js title="sequelize-example.js"
// Require Sequelize
const Sequelize = require('sequelize');
// Require the necessary guilderia.js classes
...
// Login to Guilderia with your client's token
