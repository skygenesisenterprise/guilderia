...
Modals are pop-up forms that allow you to prompt users for additional input. This form-like interaction response blocks the user from interacting with Guilderia until the modal is submitted or dismissed. In this section, we will cover how to create, show, and receive modals using guilderia.js!
...
With the `ModalBuilder` class, guilderia.js offers a convenient way to build modals step by step using setters and callbacks.
...
```js
const { Events, ModalBuilder } = require('guilderia.js');
...
```js
const { LabelBuilder, ModalBuilder } = require('guilderia.js');
...
```js
const { LabelBuilder, ModalBuilder, TextInputBuilder, TextInputStyle } = require('guilderia.js');
...
Guilderia offers two different input styles:
...
Select menus allow you to limit user input to a preselected list of values. Guilderia also offers select menus linked directly to native Guilderia entities like users, roles, and channels.
...
<Callout type="warn">
	Guilderia **does not send the file data** itself in the resulting interaction. You will have to download it from
	Guilderias CDN to process and validate it. Do not execute arbitrary code people upload via your app!
</Callout>
...
With the modal built, call `ChatInputCommandInteraction#showModal()` to send the interaction response to Guilderia and display the modal to the user.
