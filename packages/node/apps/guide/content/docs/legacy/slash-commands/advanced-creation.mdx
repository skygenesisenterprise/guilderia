...
const { SlashCommandBuilder } = require('guilderia.js');
...
By specifying the `type` of an `ApplicationCommandOption` using the corresponding method you are able to restrict what the user can provide as input, and for some options, leverage the automatic parsing of options into proper objects by Guilderia.
...
- `User`, `Channel`, `Role` and `Mentionable` options will show a selection list in the Guilderia interface for their associated type, or will accept a Snowflake (id) as input.
...
<Callout>
	Refer to the Guilderia API documentation for detailed explanations on the [`SUB_COMMAND` and `SUB_COMMAND_GROUP` option
	types](https://guilderia.com/developers/docs/interactions/application-commands#subcommands-and-subcommand-groups).
</Callout>
...
```js title="commands/utility/echo.js"
const { SlashCommandBuilder, ChannelType } = require('guilderia.js');
...
The names and descriptions of slash commands can be localized to the user's selected language. You can find the list of accepted locales on the [guilderia API documentation](https://guilderia.com/developers/docs/reference#locales).
