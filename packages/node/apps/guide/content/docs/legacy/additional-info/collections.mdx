...
guilderia.js comes with a utility class known as `Collection`.
...
<Callout>
    ...
    There is usually a reason, why a `Map` or `Collection` is used. Most structures in Guilderia can be identified with an `id`, which lends itself well to `key -> value` associations like in `Map`s.
