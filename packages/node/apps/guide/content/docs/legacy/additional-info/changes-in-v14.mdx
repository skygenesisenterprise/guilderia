...
If you previously had `@guilderiajs/builders`, `@guilderiajs/formatters`, `@guilderiajs/rest`, or `guilderia-api-types` manually installed, it's _highly_ recommended that you uninstall the packages to avoid package version conflicts.

```sh tab="npm"
npm uninstall @guilderiajs/builders @guilderiajs/formatters @guilderiajs/rest guilderia-api-types
```

```sh tab="yarn"
yarn remove @guilderiajs/builders @guilderiajs/formatters @guilderiajs/rest guilderia-api-types
```

```sh tab="pnpm"
pnpm remove @guilderiajs/builders @guilderiajs/formatters @guilderiajs/rest guilderia-api-types
```
...
guilderia.js v14 makes the switch to Guilderia API v10!
...
In addition, the old enums exported by guilderia.js v13 and lower are replaced with new enums from [guilderia-api-types](https://guilderia-api-types.dev/api/guilderia-api-types-v10).

#### New enum differences

Most of the difference between enums from guilderia.js and guilderia-api-types can be summarized as so:
...
```js
const { Client, Intents } = require('guilderia.js'); // [!code --]
const { Client, GatewayIntentBits, Partials } = require('guilderia.js'); // [!code ++]
...
```js
const { ApplicationCommandType, ApplicationCommandOptionType } = require('guilderia.js'); // [!code ++]
...
```js
const { ButtonStyle } = require('guilderia.js'); // [!code ++]
...
Some channel type guard methods that narrowed to one channel type have been removed. Instead compare the `type` property against a [ChannelType](https://guilderia-api-types.dev/api/guilderia-api-types-v10/enum/ChannelType) enum member to narrow channels.

```js
const { ChannelType } = require('guilderia.js'); // [!code ++]
...
The following properties have been removed as they are not documented by Guilderia:
...
`CommandInteractionOptionResolver#getMember()` no longer has a parameter for `required`. See [this pull request](https://github.com/guilderiajs/guilderia.js/pull/7188) for more information.
...
```js
const { Constants } = require('guilderia.js'); // [!code --]
const { Colors } = Constants; // [!code --]
const { Colors } = require('guilderia.js'); // [!code ++]
```
...
`applicationCommandCreate`, `applicationCommandDelete` and `applicationCommandUpdate` have all been removed. See [this pull request](https://github.com/guilderiajs/guilderia.js/pull/6492) for more information.
...
`Guild#me` has been moved to `GuildMemberManager#me`. See [this pull request](https://github.com/guilderiajs/guilderia.js/pull/7669) for more information.
...
`GuildMember#pending` is now nullable to account for partial guild members. See [this issue](https://github.com/guilderiajs/guilderia.js/issues/6546) for more information.
...
const { ButtonBuilder } = require('guilderia.js'); // [!code ++]
...
`ManageEmojisAndStickers` has been deprecated due to API changes. Its replacement is `ManageGuildExpressions`. See [this pull request](https://github.com/guilderia/guilderia-api-docs/pull/6017) for more information.
...
This REST event has been removed as guilderia.js now uses [Undici](https://github.com/nodejs/undici) as the underlying request handler. You must now use a [Diagnostics Channel](https://undici.nodejs.org/#/docs/api/DiagnosticsChannel). Here is a simple example:
...
`Sticker#tags` is now a nullable string (`string | null`). Previously, it was a nullable array of strings (`string[] | null`). See [this pull request](https://github.com/guilderiajs/guilderia.js/pull/8010) for more information.

### ThreadChannel

The `MAX` helper used in `ThreadAutoArchiveDuration` has been removed. Guilderia has since allowed any guild to use any auto archive time which makes this helper redundant.
...
`Util.resolveAutoArchiveMaxLimit()` has been removed. Guilderia has since allowed any guild to use any auto archive time which makes this method redundant.

Other functions in `Util` have been moved to top-level exports so you can directly import them from `guilderia.js`.

```js
import { Util } from 'guilderia.js'; // [!code --]
Util.escapeMarkdown(message); // [!code --]
import { escapeMarkdown } from 'guilderia.js'; // [!code ++]
escapeMarkdown(message); // [!code ++]
```

### `.deleted` Field(s) have been removed

You can no longer use the `deleted` property to check if a structure was deleted. See [this issue](https://github.com/guilderiajs/guilderia.js/issues/7091) for more information.
...
```js
const { ComponentType } = require('guilderia.js'); // [!code ++]
...
guilderia.js uses `@guilderiajs/ws` internally
