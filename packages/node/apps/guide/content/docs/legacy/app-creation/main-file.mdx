...
// Log in to Guilderia with your client's token
client.login(token);
```

This is how you create a client instance for your Guilderia bot and log in to Guilderia. The `GatewayIntentBits.Guilds` intents option is necessary for the guilderia.js client to work as you expect it to, as it ensures that the caches for guilds, channels, and roles are populated and available for internal use.

<Callout>The term "guild" is used by the Guilderia API and in guilderia.js to refer to a Guilderia server.</Callout>

Intents also define which events Guilderia should send to your bot, and you may wish to enable more than just the minimum. You can read more about the other intents in the [Intents topic](../popular-topics/intents).
