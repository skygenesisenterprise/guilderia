...
The command deployment script to register your slash commands with Guilderia.
...
- `clientId`: Your application's client id ([Guilderia Developer Portal](https://guilderia.com/developers/applications) > "General Information" > application id)
- `guildId`: Your development server's id ([Enable developer mode](https://support.guilderia.com/hc/articles/206346498) > Right-click the server title > "Copy ID")
...
const { REST, Routes } = require('guilderia.js');
...
Once you fill in these values, run `node deploy-commands.js` in your project directory to register your commands to the guild specified. If you see the success message, check for the commands in the server by typing `/`! If all goes well, you should be able to run them and see your bot's response in Guilderia!
...
<Callout>
    ...
    You may wish to have a separate application and token in the Guilderia Dev Portal for your dev application, to avoid duplication between your guild-based commands and the global deployment.
