...
const { Client, Collection, Events, GatewayIntentBits, MessageFlags } = require('guilderia.js');
...
Your command files are now loaded into your bot, and the event listener is prepared and ready to respond. In the next section, we cover the final step: a command deployment script you'll need to register your commands so they appear in the Guilderia client.
