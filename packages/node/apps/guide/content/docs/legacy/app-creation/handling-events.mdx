...
Node.js uses an event-driven architecture, making it possible to execute code when a specific event occurs. The guilderia.js library takes full advantage of this. You can visit the `Client` documentation to see the full list of events.
...
<Files>
	<Folder name="guilderia-bot" defaultOpen>
...
const { Events } = require('guilderia.js');
...
const { Events, MessageFlags } = require('guilderia.js');
...
const { Client, Collection, GatewayIntentBits } = require('guilderia.js');
...
<Callout>
	In most cases, you can access your `client` instance in other files by obtaining it from one of the other guilderia.js
