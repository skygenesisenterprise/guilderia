...
Now that you've installed Node, guilderia.js, and hopefully a linter, you're almost ready to start coding! The next step you need to take is setting up an actual Guilderia bot application via Guilderia's website.

It's effortless to create one. The steps you need to take are as follows:

1. Open the [Guilderia developer portal](https://guilderia.com/developers/applications) and log into your account.
...
You can edit your application's name, description, and avatar here. Once you've done that, then congratulations—you're now the proud owner of a shiny new Guilderia bot! You're not entirely done, though.
...
A token is essentially your bot's password; it's what your bot uses to login to Guilderia. With that said, **it is vital that you do not ever share this token with anybody, purposely or accidentally**. If someone does manage to get a hold of your bot's token, they can use your bot as if it were theirs—this means they can perform malicious acts with it.
...
<Callout type="error" title="Resetting Your Application Token">
	If your bot token has been compromised by committing it to a public repository, posting it in guilderia.js support etc.
