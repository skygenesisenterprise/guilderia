...
Permissions are Guilderia's primary feature, enabling users to customize their server's workings to their liking.
Essentially, Permissions and permission overwrites tell Guilderia who is allowed to do what and where.
...
To include permission checks like `Administrator` or `ManageGuild`, keep reading as we will cover Guilderia Permissions and all their intricacies in the following sections.

## Terminology

- Permission: The ability to execute a certain action in Guilderia
- Overwrite: Rule on a channel to modify the permissions for a member or role
- BitField: Binary representation of Guilderia permissions
...
<Callout>
	...
	permission calculator, you can look at the "Bot" section in the [Guilderia developer
	portal](https://guilderia.com/developers/applications).
</Callout>
...
```js
const { PermissionsBitField } = require('guilderia.js');
...
```js
const { ChannelType, PermissionsBitField } = require('guilderia.js');
...
<Callout>
	This is indeed all that a "sync" in Guilderia categories means! Users and developers alike commonly misunderstand this
...
The `PermissionsBitField` object is a guilderia.js class containing a permissions bit field and a bunch of utility methods to manipulate it easily.
...
guilderia.js provides a `toArray()` method, which can be used to convert a `Permissions` object into an array containing permission flags. This is useful if you want to display/list them and it enables you to use other array manipulation methods. For example:
...
Some methods and properties in guilderia.js return permission decimals rather than a Permissions object, making it hard to manipulate or read them if you don't want to use bitwise operations.
