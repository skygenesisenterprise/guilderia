...
Before we get into actually assigning permissions, let's quickly go over the method Guilderia uses to determine a guild member's final permissions:
...
Check out [Guilderia's help article](https://support.guilderia.com/hc/articles/219576828) if you need assistance with this.
...
Some Guilderia permissions apply implicitly based on logical use, which can cause unwanted behavior if you are not aware of this fact.
...
Let's say you want to send a message to a channel. To prevent unnecessary API calls, you want to make sure your bot's permissions in this channel include `SendMessages` (more on how to achieve this [here](./permissions#checking-for-permissions)). The check passes, but you still can't send the message and are greeted with `GuilderiaAPIError: Missing Access`.
...
During your development, you will likely run into `GuilderiaAPIError: Missing Permissions` at some point. One of the following can cause this error:
