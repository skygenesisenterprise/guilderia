...
const collectorFilter = (m) => m.content.includes('guilderia');
...
One important difference to note with interaction collectors is that Guilderia expects a response to _all_ interactions within 3 seconds - even ones that you don't want to collect. For this reason, you may wish to `.deferUpdate()` all interactions in your filter, or not use a filter at all and handle this behavior in the `collect` event.

```js
const { ComponentType } = require('guilderia.js');
...
Unlike other Promise-based collectors, this method will only ever collect one interaction that passes the filter. If no interactions are collected before the time runs out, the Promise will reject. This behavior aligns with Guilderia's requirement that actions should immediately receive a response. In this example, you will use `.deferUpdate()` on all interactions in the filter.
...
As Guilderia does not inform you if the user dismisses the modal, supplying a maximum `time` to wait for is crucial:
