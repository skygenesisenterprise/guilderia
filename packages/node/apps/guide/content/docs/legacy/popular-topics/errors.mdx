...
You can find a full list of messages [here](https://guilderia.com/developers/docs/topics/opcodes-and-status-codes#json) in the Guilderia API documentation.
...
The code is another partial representation of the message, in this case, `Invalid Form Body`. You can find a full list of codes [here](https://guilderia.com/developers/docs/topics/opcodes-and-status-codes#json-json-error-codes)

The code is also handy if you want only to handle a specific error. Say you're trying to delete a message which may or may not be there, and wanted to ignore unknown message errors. This can be done by checking the code, either manually or using guilderia.js constants.
...
```js
const { RESTJSONErrorCodes } = require('guilderia.js');
...
You can find a list of constants [here](https://guilderia-api-types.dev/api/guilderia-api-types-v10/enum/RESTJSONErrorCodes).
...
## Common guilderia.js and API errors
...
<Callout>
	**Before** the release of **version 12**, there used to be an issue where the token was not prefixed correctly, which
	...
	sure you have updated guilderia.js to the current stable version.
</Callout>
...
```js
const { Client, Events, GatewayIntentBits } = require('guilderia.js');
...
- The `GuildMembers` intent is not specified or enabled in the [developer portal](https://guilderia.com/developers/applications).
...
- Make sure to handle the returned promise gracefully. Guilderia may change the exact values of this rate limit in the future. You can find all relevant details in the caught error.
...
In the case of the last two reasons, the error is not preventable, as the Guilderia API does not provide a way to check if you can send a user a DM until you attempt to send one. The best way to handle this error is to add a `.catch()` where you try to DM the user and either ignore the rejected Promise or do what you want because of it.
