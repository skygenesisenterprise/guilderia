...
The `BroadcastEvalOptions` typedef was introduced in guilderia.js v13 as the second parameter in `.broadcastEval()`. It accepts two properties: `shard` and `context`. The `context` property will be sent as the second argument to your function.
