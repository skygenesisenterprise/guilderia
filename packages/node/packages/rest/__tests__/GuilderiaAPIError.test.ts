...
import { GuilderiaAPIError } from '../src/index.js';

test('Unauthorized', () => {
	const error = new GuilderiaAPIError(
		{ message: '401: Unauthorized', code: 0 },
		0,
		401,
		'PATCH',
		'https://guilderia.com/api/v10/guilds/:id',
		{
			files: undefined,
			body: undefined,
		},
	...
	expect(error.name).toEqual('GuilderiaAPIError[0]');
	expect(error.status).toEqual(401);
	expect(error.url).toEqual('https://guilderia.com/api/v10/guilds/:id');
...
test('Invalid Form Body Error (error.{property}._errors.{index})', () => {
	const error = new GuilderiaAPIError(
		...
		400,
		'PATCH',
		'https://guilderia.com/api/v10/users/@me',
		{
			files: undefined,
			body: {
				username: 'a',
	...
	expect(error.method).toEqual('PATCH');
	expect(error.name).toEqual('GuilderiaAPIError[50035]');
	expect(error.status).toEqual(400);
	expect(error.url).toEqual('https://guilderia.com/api/v10/users/@me');
	expect(error.requestBody.files).toBeUndefined();
...
test('Invalid FormFields Error (error.errors.{property}.{property}.{index}.{property}._errors.{index})', () => {
	const error = new GuilderiaAPIError(
		...
		400,
		'POST',
		'https://guilderia.com/api/v10/channels/:id',
		{},
	);
	...
	expect(error.name).toEqual('GuilderiaAPIError[50035]');
	expect(error.status).toEqual(400);
	expect(error.url).toEqual('https://guilderia.com/api/v10/channels/:id');
});
...
test('Invalid Oauth Code Error (error.error)', () => {
	const error = new GuilderiaAPIError(
		...
		400,
		'POST',
		'https://guilderia.com/api/v10/oauth2/token',
		{
	...
	expect(error.method).toEqual('POST');
	expect(error.name).toEqual('GuilderiaAPIError[invalid_request]');
	expect(error.status).toEqual(400);
	expect(error.url).toEqual('https://guilderia.com/api/v10/oauth2/token');
});
