import type { Gateway, GatewayDispatchPayload, GatewaySendPayload, GatewayDispatchEvents } from '@guilderiajs/core';
import type { ManagerShardEventsMap, WebSocketShardEvents } from '@guilderiajs/ws';
import { AsyncEventEmitter } from '@vladfrangu/async_event_emitter';
import type { PubSubRedisBroker } from './PubSubRedis.js';

export type GuilderiaEvents = {
...
interface Events extends GuilderiaEvents {
	// eslint-disable-next-line @typescript-eslint/no-use-before-define
	[RedisGateway.GatewaySendEvent]: GatewaySendPayload;
}

export type RedisBrokerGuilderiaEvents = {
 ...
 * - You can also opt to use the class as-is without `@guilderiajs/core`, if you so desire. Events are properly typed
 ...
 * - One drawback to using this directly with `@guilderiajs/core` is that you lose granular control over when to `ack`
 ...
 * import { RedisGateway, PubSubRedisBroker, kUseRandomGroupName } from '@guilderiajs/brokers';
 ...
 * const gateway = new WebSocketManager(gatewayOptionsHere); // see @guilderiajs/ws for examples.
 *
 ...
 *
 * // listen to payloads we should send to Guilderia
...
{
	/**
	 * Event used over the broker used to tell shards to send a payload to Guilderia.
	 */
	public static readonly GatewaySendEvent = 'gateway_send' as const;

	/**
	 * Converts a dispatch event from `@guilderiajs/ws` to arguments for a `broker.publish` call.
	 */
	...
	public constructor(
		private readonly broker: PubSubRedisBroker<RedisBrokerGuilderiaEvents>,
