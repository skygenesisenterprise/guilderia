...
const { ApplicationCommandOptionType } = require('guilderia-api-types/v10');
...
class CommandInteractionOptionResolver {
  ...
  get(name, required = false) {
    ...
    if (!option) {
      if (required) {
        throw new GuilderiajsTypeError(ErrorCodes.CommandInteractionOptionNotFound, name);
      }
  ...
  _getTypedOption(name, allowedTypes, properties, required) {
    ...
    if (!option) {
      ...
      throw new GuilderiajsTypeError(ErrorCodes.CommandInteractionOptionType, name, option.type, allowedTypes.join(', '));
    } else if (required && properties.every(prop => option[prop] === null || option[prop] === undefined)) {
      throw new GuilderiajsTypeError(ErrorCodes.CommandInteractionOptionEmpty, name, option.type);
    }
  ...
  getSubcommand(required = true) {
    if (required && !this._subcommand) {
      throw new GuilderiajsTypeError(ErrorCodes.CommandInteractionOptionNoSubcommand);
    }
  ...
  getSubcommandGroup(required = false) {
    if (required && !this._group) {
      throw new GuilderiajsTypeError(ErrorCodes.CommandInteractionOptionNoSubcommandGroup);
    }
  ...
  getFocused() {
    ...
    if (!focusedOption) throw new GuilderiajsTypeError(ErrorCodes.AutocompleteInteractionOptionNoFocusedOption);
