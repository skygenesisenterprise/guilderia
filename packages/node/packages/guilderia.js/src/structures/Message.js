...
const { messageLink } = require('@guilderiajs/formatters');
const { GuilderiaSnowflake } = require('@sapphire/snowflake');
const {
  InteractionType,
  ChannelType,
  MessageType,
  ...
  PermissionFlagsBits,
} = require('guilderia-api-types/v10');
const { GuilderiajsError, ErrorCodes } = require('../errors/index.js');
...
class Message extends Base {
  ...
  _patch(data) {
     ...
     * @type {number}
     */
    this.createdTimestamp = GuilderiaSnowflake.timestampFrom(this.id);
    ...
    if ('type' in data) {
      ...
      /**
       * Whether or not this message was sent by Guilderia, not actually a user (e.g. pin notifications)
       *
       * @type {?boolean}
       */
    ...
    // Guilderia sends null if the message has not been edited
     ...
     *
     * @see {@link https://guilderia.com/developers/docs/resources/message#message-object-message-types}
  ...
  async awaitMessageComponent(options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      collector.once('end', (interactions, reason) => {
        ...
        else reject(new GuilderiajsError(ErrorCodes.InteractionCollectorError, reason));
      });
    });
  }
  ...
  async fetchReference() {
    if (!this.reference) throw new GuilderiajsError(ErrorCodes.MessageReferenceMissing);
    const { channelId, messageId } = this.reference;
    if (!messageId) throw new GuilderiajsError(ErrorCodes.MessageReferenceMissing);
    const channel = this.client.channels.resolve(channelId);
    if (!channel) throw new GuilderiajsError(ErrorCodes.GuildChannelResolve);
   ...
   */
  async edit(options) {
    if (!this.channel) throw new GuilderiajsError(ErrorCodes.ChannelNotCached);
  ...
  async startThread(options = {}) {
    ...
    if (![ChannelType.GuildText, ChannelType.GuildAnnouncement].includes(this.channel.type)) {
      throw new GuilderiajsError(ErrorCodes.MessageThreadParent);
    }

    if (this.hasThread) throw new GuilderiajsError(ErrorCodes.MessageExistingThread);
   ...
   */
  async fetchWebhook() {
    if (!this.webhookId) throw new GuilderiajsError(ErrorCodes.WebhookMessage);
    if (this.webhookId === this.applicationId) throw new GuilderiajsError(ErrorCodes.WebhookApplication);
