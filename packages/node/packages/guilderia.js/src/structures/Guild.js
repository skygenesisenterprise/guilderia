...
const { ChannelType, GuildPremiumTier, Routes, GuildFeature } = require('guilderia-api-types/v10');
...
const { guilderiaSort, getSortableGroupTypes, resolvePartialEmoji } = require('../util/Util.js');
...
/**
 * Represents a guild (or a server) on Guilderia.
...
class Guild extends AnonymousGuild {
  ...
  async fetchOwner(options) {
    if (!this.ownerId) {
      throw new GuilderiajsError(ErrorCodes.FetchOwnerId, 'guild');
    }
  ...
  /**
   * Obtains a guild preview for this guild from Guilderia.
   *
   ...
   *   .then(res => {
   *     console.log(`Vanity URL: https://guilderia.gg/${res.code} with ${res.uses} uses`);
   *   })
   *   .catch(console.error);
   */
    ...
    const newData = await this.client.rest.put(Routes.guildOnboarding(this.id), {
      body: {
        prompts: options.prompts?.map(prompt => ({
          ...
          id: prompt.id ?? GuilderiaSnowflake.generate().toString(),
          title: prompt.title,
  ...
  /**
   * The voice state adapter for this guild that can be used with `@guilderiajs/voice` to play audio in voice
   * and stage channels.
   *
   * @type {Function}
   * @readonly
   ...
   */
  _sortedRoles() {
    return guilderiaSort(this.roles.cache);
  }
