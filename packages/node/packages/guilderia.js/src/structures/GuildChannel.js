...
const { PermissionFlagsBits, ChannelType } = require('guilderia-api-types/v10');
...
class GuildChannel extends BaseChannel {
  ...
  _patch(data) {
    ...
    if ('position' in data) {
      /**
       * The raw position of the channel from Guilderia
       *
       * @type {number}
       */
  ...
  async lockPermissions() {
    if (!this.parent) throw new GuilderiajsError(ErrorCodes.GuildChannelOrphan);
