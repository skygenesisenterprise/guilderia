...
class VoiceState extends Base {
   ...
   */
  async edit(options) {
    if (this.channel?.type !== ChannelType.GuildStageVoice) throw new GuilderiajsError(ErrorCodes.VoiceNotStageChannel);

    const target = this.client.user.id === this.id ? '@me' : this.id;

    if (target !== '@me' && options.requestToSpeak !== undefined) {
      throw new GuilderiajsError(ErrorCodes.VoiceStateNotOwn);
    }

    if (!['boolean', 'undefined'].includes(typeof options.requestToSpeak)) {
      throw new GuilderiajsTypeError(ErrorCodes.VoiceStateInvalidType, 'requestToSpeak');
    }

    if (!['boolean', 'undefined'].includes(typeof options.suppressed)) {
      throw new GuilderiajsTypeError(ErrorCodes.VoiceStateInvalidType, 'suppressed');
    }
