...
const { InteractionType, Routes } = require('guilderia-api-types/v10');
const { GuilderiajsTypeError, GuilderiajsError, ErrorCodes } = require('../../errors/index.js');
...
class TextBasedChannel {
   ...
   * (see {@link https://guilderia.com/developers/docs/resources/message#allowed-mentions-object here} for more details)
  ...
  /**
   * Options provided to control parsing of mentions by Guilderia
   *
   ...
   * // Send a remote file
   * channel.send({
   *   files: ['https://github.com/guilderiajs.png']
   * })
   *   .then(console.log)
   *   .catch(console.error);
   * @example
   ...
   * const filter = message => message.content.includes('guilderia');
  ...
  async bulkDelete(messages, filterOld = false) {
    if (Array.isArray(messages) || messages instanceof Collection) {
      ...
      if (filterOld) {
        messageIds = messageIds.filter(
          id => Date.now() - GuilderiaSnowflake.timestampFrom(id) < MaxBulkDeletableMessageAge,
        );
      }
    ...
    throw new GuilderiajsTypeError(ErrorCodes.MessageBulkDeleteType);
  }
