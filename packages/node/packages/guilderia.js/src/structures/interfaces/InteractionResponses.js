...
const { isJSONEncodable } = require('@guilderiajs/util');
const { InteractionResponseType, MessageFlags, Routes, InteractionType } = require('guilderia-api-types/v10');
...
class InteractionResponses {
  ...
  async deferReply(options = {}) {
    if (this.deferred || this.replied) throw new GuilderiajsError(ErrorCodes.InteractionAlreadyReplied);
  ...
  async editReply(options) {
    if (!this.deferred && !this.replied) throw new GuilderiajsError(ErrorCodes.InteractionNotReplied);
  ...
  async awaitModalSubmit(options) {
    if (typeof options.time !== 'number') throw new GuilderiajsError(ErrorCodes.InvalidType, 'time', 'number');
