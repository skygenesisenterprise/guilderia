...
const { ComponentType } = require('guilderia-api-types/v10');
...
class ModalComponentResolver {
  ...
  getComponent(customId) {
    ...
    if (!component) throw new GuilderiajsTypeError(ErrorCodes.ModalSubmitInteractionComponentNotFound, customId);
  ...
  _getTypedComponent(customId, allowedTypes, properties, required) {
    ...
    if (!allowedTypes.includes(component.type)) {
      ...
      throw new GuilderiajsTypeError(ErrorCodes.ModalSubmitInteractionComponentEmpty, customId, component.type);
    }
