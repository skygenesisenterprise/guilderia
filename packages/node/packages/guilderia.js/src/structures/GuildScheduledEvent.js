...
const { GuildScheduledEventStatus, GuildScheduledEventEntityType, RouteBases } = require('guilderia-api-types/v10');
...
class GuildScheduledEvent extends Base {
  ...
  _patch(data) {
     ...
     *
     * @see {@link https://guilderia.com/developers/docs/resources/guild-scheduled-event#guild-scheduled-event-object-guild-scheduled-event-entity-metadata}
  ...
  get createdTimestamp() {
    return GuilderiaSnowflake.timestampFrom(this.id);
  }
  ...
  async createInviteURL(options) {
    ...
    if (this.entityType === GuildScheduledEventEntityType.External) {
      if (!options?.channel) throw new GuilderiajsError(ErrorCodes.InviteOptionsMissingChannel);
      channelId = this.guild.channels.resolveId(options.channel);
      if (!channelId) throw new GuilderiajsError(ErrorCodes.GuildChannelResolve);
    }
   ...
   * @returns {string}
   * @example
   * // Logs: Event: https://guilderia.com/events/412345678901234567/499876543211234567
