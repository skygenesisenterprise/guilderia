...
const { ChannelFlags, ChannelType, PermissionFlagsBits, Routes } = require('guilderia-api-types/v10');
const { GuilderiajsRangeError, ErrorCodes } = require('../errors/index.js');
...
/**
 * Represents a thread channel on Guilderia.
 *
 * @extends {BaseChannel}
...
class ThreadChannel extends BaseChannel {
  ...
  async setInvitable(invitable = true, reason = undefined) {
    if (this.type !== ChannelType.PrivateThread) {
      throw new GuilderiajsRangeError(ErrorCodes.ThreadInvitableType, this.type);
    }
