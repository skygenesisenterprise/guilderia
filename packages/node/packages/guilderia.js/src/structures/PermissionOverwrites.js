...
const { OverwriteType } = require('guilderia-api-types/v10');
...
class PermissionOverwrites extends Base {
  ...
  static resolve(overwrite, guild) {
    ...
    if (!id) {
      throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'overwrite.id', 'UserResolvable or RoleResolvable');
    }

    if (overwrite.type !== undefined && (typeof overwrite.type !== 'number' || !(overwrite.type in OverwriteType))) {
      throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'overwrite.type', 'OverwriteType', true);
    }

    let type;
    if (typeof overwrite.id === 'string') {
      if (overwrite.type === undefined) {
        throw new GuilderiajsTypeError(ErrorCodes.PermissionOverwritesTypeMandatory);
      }
      ...
      if (overwrite.type !== undefined && type !== overwrite.type) {
        throw new GuilderiajsTypeError(ErrorCodes.PermissionOverwritesTypeMismatch, OverwriteType[type]);
      }
    }
