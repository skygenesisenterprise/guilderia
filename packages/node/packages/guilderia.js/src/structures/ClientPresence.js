...
const { GatewayOpcodes, ActivityType } = require('guilderia-api-types/v10');
...
class ClientPresence extends Presence {
  ...
  /**
   * Parses presence data into a packet ready to be sent to Guilderia
   *
    ...
    if (activities?.length) {
      for (const [i, activity] of activities.entries()) {
        if (typeof activity.name !== 'string') {
          throw new GuilderiajsTypeError(ErrorCodes.InvalidType, `activities[${i}].name`, 'string');
        }
