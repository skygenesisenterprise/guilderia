...
const { calculateShardId } = require('@guilderiajs/util');
...
class ShardClientUtil {
  ...
  async broadcastEval(script, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      if (typeof script !== 'function') {
        reject(new GuilderiajsTypeError(ErrorCodes.ShardingInvalidEvalBroadcast));
        return;
      }
  ...
  static shardIdForGuildId(guildId, shardCount) {
    ...
    if (shard < 0) throw new GuilderiajsError(ErrorCodes.ShardingShardMiscalculation, shard, guildId, shardCount);
    return shard;
  }
