...
class Shard extends AsyncEventEmitter {
  constructor(manager, id) {
    ...
    this.env = {
      ...
      GUILDERIA_TOKEN: this.manager.token,
    };
  ...
  async spawn(timeout = 30_000) {
    if (this.process) throw new GuilderiajsError(ErrorCodes.ShardingProcessExists, this.id);
    if (this.worker) throw new GuilderiajsError(ErrorCodes.ShardingWorkerExists, this.id);
    ...
    return new Promise((resolve, reject) => {
      ...
      const onDisconnect = () => {
        cleanup();
        reject(new GuilderiajsError(ErrorCodes.ShardingReadyDisconnected, this.id));
      };

      const onDeath = () => {
        cleanup();
        reject(new GuilderiajsError(ErrorCodes.ShardingReadyDied, this.id));
      };

      const onTimeout = () => {
        cleanup();
        reject(new GuilderiajsError(ErrorCodes.ShardingReadyTimeout, this.id));
      };
  ...
  async fetchClientValue(prop) {
    ...
    if (!this.process && !this.worker) {
      throw new GuilderiajsError(ErrorCodes.ShardingNoChildExists, this.id);
    }
