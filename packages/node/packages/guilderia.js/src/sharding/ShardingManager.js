...
const { range } = require('@guilderiajs/util');
const { AsyncEventEmitter } = require('@vladfrangu/async_event_emitter');
const { GuilderiajsError, GuilderiajsTypeError, GuilderiajsRangeError, ErrorCodes } = require('../errors/index.js');
 ...
 * with sharding. It can spawn a specific number of shards or the amount that Guilderia suggests for the bot, and takes a
...
class ShardingManager extends AsyncEventEmitter {
  ...
  constructor(file, options) {
     ...
     */
    this.file = file;
    if (!file) throw new GuilderiajsError(ErrorCodes.ClientInvalidOption, 'File', 'specified.');
    if (!path.isAbsolute(file)) this.file = path.resolve(process.cwd(), file);
    // eslint-disable-next-line n/no-sync
    const stats = fs.statSync(this.file);
    if (!stats.isFile()) throw new GuilderiajsError(ErrorCodes.ClientInvalidOption, 'File', 'a file');
    ...
    if (this.shardList !== 'auto') {
      if (!Array.isArray(this.shardList)) {
        throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'shardList', 'an array.');
      }

      this.shardList = [...new Set(this.shardList)];
      if (this.shardList.length < 1) {
        throw new GuilderiajsRangeError(ErrorCodes.ClientInvalidOption, 'shardList', 'at least 1 id.');
      }
      ...
      if (
        ...
        throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'shardList', 'an array of positive integers.');
      }
    }
    ...
    if (this.totalShards !== 'auto') {
      if (typeof this.totalShards !== 'number' || Number.isNaN(this.totalShards)) {
        throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'Amount of shards', 'a number.');
      }

      if (this.totalShards < 1) {
        throw new GuilderiajsRangeError(ErrorCodes.ClientInvalidOption, 'Amount of shards', 'at least 1.');
      }

      if (!Number.isInteger(this.totalShards)) {
        throw new GuilderiajsRangeError(ErrorCodes.ClientInvalidOption, 'Amount of shards', 'an integer.');
      }
    }
    ...
    if (this.mode !== 'process' && this.mode !== 'worker') {
      throw new GuilderiajsRangeError(ErrorCodes.ClientInvalidOption, 'Sharding mode', '"process" or "worker"');
    }
    ...
    process.env.GUILDERIA_TOKEN = this.token;
  }
  ...
  async spawn({ amount = this.totalShards, delay = 5_500, timeout = 30_000 } = {}) {
    ...
    if (shardAmount === 'auto') {
      ...
      if (!Number.isInteger(shardAmount)) {
        throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'Amount of shards', 'an integer.');
      }
    }

    // Make sure this many shards haven't already been spawned
    if (this.shards.size >= shardAmount) throw new GuilderiajsError(ErrorCodes.ShardingAlreadySpawned, this.shards.size);
    ...
    if (this.shardList.some(shardId => shardId >= shardAmount)) {
      throw new GuilderiajsRangeError(
  ...
  async broadcastEval(script, options = {}) {
    if (typeof script !== 'function') {
      throw new GuilderiajsTypeError(ErrorCodes.ShardingInvalidEvalBroadcast);
    }
  ...
  async _performOnShards(method, args, shard) {
    if (this.shards.size === 0) throw new GuilderiajsError(ErrorCodes.ShardingNoShards);

    if (typeof shard === 'number') {
      if (this.shards.has(shard)) return this.shards.get(shard)[method](...args);
      throw new GuilderiajsError(ErrorCodes.ShardingShardNotFound, shard);
    }

    if (this.shards.size !== this.shardList.length) {
      throw new GuilderiajsError(ErrorCodes.ShardingInProcess);
    }
