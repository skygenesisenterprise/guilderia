...
const { Collection } = require('@guilderiajs/collection');
const { REST, RESTEvents, makeURLSearchParams } = require('@guilderiajs/rest');
const { WebSocketManager, WebSocketShardEvents, WebSocketShardStatus } = require('@guilderiajs/ws');
const { AsyncEventEmitter } = require('@vladfrangu/async_event_emitter');
const { GatewayDispatchEvents, GatewayIntentBits, OAuth2Scopes, Routes } = require('guilderia-api-types/v10');
const { GuilderiajsError, GuilderiajsTypeError, ErrorCodes } = require('../errors/index.js');
...
/**
 * The main hub for interacting with the Guilderia API, and the starting point for any bot.
 *
...
class Client extends AsyncEventEmitter {
  ...
  constructor(options) {
    ...
    if (typeof options !== 'object' || options === null) {
      throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'options', 'object', true);
    }
    ...
    if (!this.token && 'GUILDERIA_TOKEN' in process.env) {
      /**
       * Authorization token for the logged in bot.
       * If present, this defaults to `process.env.GUILDERIA_TOKEN` when instantiating the client
       * <warn>This should be kept private at all times.</warn>
       *
       * @type {?string}
       */
      this.token = process.env.GUILDERIA_TOKEN;
  ...
  /**
   * Logs the client in, establishing a WebSocket connection to Guilderia.
   *
  ...
  async login(token = this.token) {
    if (!token || typeof token !== 'string') throw new GuilderiajsError(ErrorCodes.TokenInvalid);
  ...
  /**
   * Attaches event handlers to the WebSocketShardManager from `@guilderiajs/ws`.
   *
   * @private
   */
  _attachEvents() {
  ...
  /**
   * Options used when fetching an invite from Guilderia.
   *
  ...
  /**
   * Obtains an invite from Guilderia.
   *
   * @param {InviteResolvable} invite Invite code or URL
   * @param {ClientFetchInviteOptions} [options] Options for fetching the invite
   * @returns {Promise<Invite>}
   * @example
   * client.fetchInvite('https://guilderia.gg/djs')
  ...
  /**
   * Obtains a template from Guilderia.
   *
   * @param {GuildTemplateResolvable} template Template code or URL
   * @returns {Promise<GuildTemplate>}
   * @example
   * client.fetchGuildTemplate('https://guilderia.new/FKvmczH2HyUf')
  ...
  /**
   * Obtains a webhook from Guilderia.
   *
   * @param {Snowflake} id The webhook's id
  ...
  /**
   * Obtains the available voice regions from Guilderia.
   *
  ...
  /**
   * Obtains a sticker from Guilderia.
   *
  ...
  /**
   * Obtains a guild preview from Guilderia, available for all guilds the bot is in and all Discoverable guilds.
   *
  ...
  async fetchGuildPreview(guild) {
    ...
    if (!id) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'guild', 'GuildResolvable');
    const data = await this.rest.get(Routes.guildPreview(id));
    return new GuildPreview(this, data);
  }

  /**
   * Obtains the widget data of a guild from Guilderia, available for guilds with the widget enabled.
   *
  ...
  generateInvite(options = {}) {
    if (typeof options !== 'object') throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'options', 'object', true);
    if (!this.application) throw new GuilderiajsError(ErrorCodes.ClientNotReady, 'generate an invite link');

    const { scopes } = options;
    if (scopes === undefined) {
      throw new GuilderiajsTypeError(ErrorCodes.InvalidMissingScopes);
    }

    if (!Array.isArray(scopes)) {
      throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'scopes', 'Array of Invite Scopes', true);
    }
    ...
    if (!scopes.includes(OAuth2Scopes.Bot) && options.permissions) {
      throw new GuilderiajsTypeError(ErrorCodes.InvalidScopesWithPermissions);
    }

    const validScopes = Object.values(OAuth2Scopes);
    const invalidScope = scopes.find(scope => !validScopes.includes(scope));
    if (invalidScope) {
      throw new GuilderiajsTypeError(ErrorCodes.InvalidElement, 'Array', 'scopes', invalidScope);
    }
    ...
    if (options.guild) {
      ...
      if (!guildId) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'options.guild', 'GuildResolvable');
  ...
  _validateOptions(options = this.options) {
    if (options.intents === undefined && options.ws?.intents === undefined) {
      throw new GuilderiajsTypeError(ErrorCodes.ClientMissingIntents);
    } else {
      options.intents = new IntentsBitField(options.intents ?? options.ws.intents).freeze();
    }

    if (typeof options.sweepers !== 'object' || options.sweepers === null) {
      throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'sweepers', 'an object');
    }

    if (!Array.isArray(options.partials)) {
      throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'partials', 'an Array');
    }

    if (typeof options.waitGuildTimeout !== 'number' || Number.isNaN(options.waitGuildTimeout)) {
      throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'waitGuildTimeout', 'a number');
    }

    if (typeof options.failIfNotExists !== 'boolean') {
      throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'failIfNotExists', 'a boolean');
    }

    if (typeof options.enforceNonce !== 'boolean') {
      throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'enforceNonce', 'a boolean');
    }
    ...
    if (
      ...
      throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'allowedMentions', 'an object');
    }

    if (typeof options.ws !== 'object' || options.ws === null) {
      throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'ws', 'an object');
    }
    ...
    if (
      ...
      throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'presence', 'an object');
    } else {
      options.ws.initialPresence = options.ws.initialPresence ?? this.presence._parse(this.options.presence);
    }

    if (typeof options.rest !== 'object' || options.rest === null) {
      throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'rest', 'an object');
    }

    if (typeof options.jsonTransformer !== 'function') {
      throw new GuilderiajsTypeError(ErrorCodes.ClientInvalidOption, 'jsonTransformer', 'a function');
    }
  }
 ...
 *  number of milliseconds since Guilderia epoch  worker  pid    increment
 * ```
 *
...
/**
 * @external Collection
 * @see {@link https://guilderia.js.org/docs/packages/collection/stable/Collection:Class}
 */

/**
 * @external REST
 * @see {@link https://guilderia.js.org/docs/packages/rest/stable/REST:Class}
 */

/**
 * @external ImageURLOptions
 * @see {@link https://guilderia.js.org/docs/packages/rest/stable/ImageURLOptions:Interface}
 */

/**
 * @external EmojiURLOptions
 * @see {@link https://guilderia.js.org/docs/packages/rest/stable/EmojiURLOptions:TypeAlias}
 */

/**
 * @external BaseImageURLOptions
 * @see {@link https://guilderia.js.org/docs/packages/rest/stable/BaseImageURLOptions:Interface}
 */
