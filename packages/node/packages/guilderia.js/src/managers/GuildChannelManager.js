...
const { ChannelType, Routes } = require('guilderia-api-types/v10');
...
class GuildChannelManager extends CachedManager {
  ...
  async addFollower(channel, targetChannel, reason) {
    ...
    if (!channelId) {
      throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'channel', 'AnnouncementChannelResolvable');
    }

    const targetChannelId = this.resolveId(targetChannel);
    if (!targetChannelId) {
      throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'targetChannel', 'TextChannelResolvable');
    }
  ...
  async createWebhook({ channel, name, avatar, reason }) {
    const channelId = this.resolveId(channel);
    if (!channelId) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'channel', 'GuildChannelResolvable');
  ...
  async edit(channel, options) {
    ...
    if (!resolvedChannel) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'channel', 'GuildChannelResolvable');
  ...
  /**
   * Obtains one or more guild channels from Guilderia, or the channel cache if they're already available.
   *
  ...
  async fetch(id, { cache = true, force = false } = {}) {
    ...
    if (id) {
      ...
      if (this.guild.id !== innerData.guild_id) throw new GuilderiajsError(ErrorCodes.GuildChannelUnowned);
  ...
  async fetchWebhooks(channel) {
    ...
    if (!id) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'channel', 'GuildChannelResolvable');
