...
const { Routes } = require('guilderia-api-types/v10');
...
class GuildBanManager extends CachedManager {
  ...
  async fetch(options) {
    ...
    if (!before && !after && !limit && cache === undefined) {
      throw new GuilderiajsError(ErrorCodes.FetchBanResolveId);
    }
  ...
  async create(user, options = {}) {
    ...
    if (!id) throw new GuilderiajsError(ErrorCodes.BanResolveId, true);
  ...
  async remove(user, reason) {
    const id = this.client.users.resolveId(user);
    if (!id) throw new GuilderiajsError(ErrorCodes.BanResolveId);
  ...
  async bulkCreate(users, options = {}) {
    if (!users || !(Array.isArray(users) || users instanceof Collection)) {
      throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'users', 'Array or Collection of UserResolvable', true);
    }

    if (typeof options !== 'object') throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'options', 'object', true);

    const userIds = users.map(user => this.client.users.resolveId(user));
    if (userIds.length === 0) throw new GuilderiajsError(ErrorCodes.BulkBanUsersOptionEmpty);
