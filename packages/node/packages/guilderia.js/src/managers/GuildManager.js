...
const { GatewayOpcodes, Routes, RouteBases } = require('guilderia-api-types/v10');
...
class GuildManager extends CachedManager {
  ...
  /**
   * Obtains one or multiple guilds from Guilderia, or the guild cache if it's already available.
   *
  ...
  async fetchSoundboardSounds({ guildIds, time = 10_000 }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        this.client.removeListener(Events.SoundboardSounds, handler);
        this.client.decrementMaxListeners();
        reject(new GuilderiajsError(ErrorCodes.GuildSoundboardSoundsTimeout));
      }, time).unref();
