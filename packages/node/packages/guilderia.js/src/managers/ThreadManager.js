...
class ThreadManager extends CachedManager {
  ...
  /**
   * Obtains a thread from Guilderia, or the channel cache if it's already available.
   *
  ...
  async fetch(options, { cache, force } = {}) {
    ...
    if (channel) {
      ...
      if (threadChannel.parentId !== this.channel.id) throw new GuilderiajsTypeError(ErrorCodes.NotAThreadOfParent);
  ...
  /**
   * Obtains a set of archived threads from Guilderia.
    ...
    if (before !== undefined) {
      if (before instanceof ThreadChannel || /^\d{17,19}$/.test(String(before))) {
        ...
        try {
          ...
          throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'before', 'DateResolvable or ThreadChannelResolvable');
        }
      }
    }
  ...
  static _mapThreads(rawThreads, client, { parent, guild, cache }) {
    ...
    // Guilderia sends the thread id as id in this object
