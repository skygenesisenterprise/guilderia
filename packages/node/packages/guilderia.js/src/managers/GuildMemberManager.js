...
const { makeURLSearchParams } = require('@guilderiajs/rest');
const { GatewayRateLimitError } = require('@guilderiajs/util');
const { WebSocketShardEvents } = require('@guilderiajs/ws');
const { GuilderiaSnowflake } = require('@sapphire/snowflake');
const { Routes, GatewayOpcodes, GatewayDispatchEvents } = require('guilderia-api-types/v10');
...
class GuildMemberManager extends CachedManager {
  ...
  async add(user, options) {
    ...
    if (!userId) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'user', 'UserResolvable');
    if (!options.force) {
    ...
    if (options.roles) {
      ...
      for (const role of options.roles.values()) {
        ...
        if (!resolvedRole) {
          throw new GuilderiajsTypeError(ErrorCodes.InvalidElement, 'Array or Collection', 'options.roles', role);
        }
  ...
  async _fetchMany({
    ...
    query: initialQuery,
    time = 120e3,
    nonce = GuilderiaSnowflake.generate().toString(),
  } = {}) {
    if (nonce.length > 32) throw new GuilderiajsRangeError(ErrorCodes.MemberFetchNonceLength);
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        cleanup();
        reject(new GuilderiajsError(ErrorCodes.GuildMembersTimeout));
      }, time).unref();
  ...
  async edit(user, { reason, ...options }) {
    ...
    if (!id) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'user', 'UserResolvable');

    if (options.channel) {
      options.channel = this.guild.channels.resolve(options.channel);
      if (!(options.channel instanceof BaseGuildVoiceChannel)) {
        throw new GuilderiajsError(ErrorCodes.GuildVoiceChannelResolve);
      }
  ...
  async prune({ days, dry = false, count: compute_prune_count, roles = [], reason } = {}) {
    if (typeof days !== 'number') throw new GuilderiajsTypeError(ErrorCodes.PruneDaysType);
    ...
    for (const role of roles) {
      const resolvedRole = this.guild.roles.resolveId(role);
      if (!resolvedRole) {
        throw new GuilderiajsTypeError(ErrorCodes.InvalidElement, 'Array', 'options.roles', role);
      }
