...
const { ApplicationCommandPermissionType, RESTJSONErrorCodes, Routes } = require('guilderia-api-types/v10');
...
class ApplicationCommandPermissionsManager extends BaseManager {
   ...
   * {@link https://guilderia.com/developers/docs/interactions/application-commands#application-command-permissions-object-application-command-permissions-constants permission constant}.
  ...
  async set({ guild, command, permissions, token } = {}) {
    if (!token) {
      throw new GuilderiajsError(ErrorCodes.ApplicationCommandPermissionsTokenMissing);
    }
    ...
    if (!Array.isArray(permissions)) {
      throw new GuilderiajsTypeError(
  ...
  async remove({ guild, command, users, roles, channels, token } = {}) {
    ...
    if (!users && !roles && !channels) {
      throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'users OR roles OR channels', 'Array or Resolvable', true);
    }
    ...
    if (Array.isArray(users)) {
      for (const user of users) {
        ...
        if (!userId) throw new GuilderiajsTypeError(ErrorCodes.InvalidElement, 'Array', 'users', user);
    ...
    if (Array.isArray(roles)) {
      for (const role of roles) {
        ...
        if (!this.guild) throw new GuilderiajsError(ErrorCodes.GuildUncachedEntityResolve, 'roles');
        const roleId = this.guild.roles.resolveId(role);
        if (!roleId) throw new GuilderiajsTypeError(ErrorCodes.InvalidElement, 'Array', 'users', role);
    ...
    if (Array.isArray(channels)) {
      for (const channel of channels) {
        ...
        if (!this.guild) throw new GuilderiajsError(ErrorCodes.GuildUncachedEntityResolve, 'channels');
        const channelId = this.guild.channels.resolveId(channel);
        if (!channelId) throw new GuilderiajsTypeError(ErrorCodes.InvalidElement, 'Array', 'channels', channel);
  ...
  async has({ guild, command, permissionId, permissionType }) {
    const { guildId, commandId } = this._validateOptions(guild, command);
    if (!commandId) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'command', 'ApplicationCommandResolvable');
  ...
  _validateOptions(guild, command) {
    ...
    if (!guildId) throw new GuilderiajsError(ErrorCodes.GlobalCommandPermissions);
    ...
    if (command && !commandId) {
      ...
      if (!commandId) {
        throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'command', 'ApplicationCommandResolvable', true);
      }
    }
