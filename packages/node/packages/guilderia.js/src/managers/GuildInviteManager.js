...
class GuildInviteManager extends CachedManager {
  ...
  /**
   * Fetches invite(s) from Guilderia.
   *
  ...
  async fetch(options) {
    ...
    if (typeof options === 'string') {
      ...
      if (!code) throw new GuilderiajsError(ErrorCodes.InviteResolveCode);
    ...
    if (!options.code) {
      if (options.channelId) {
        ...
        if (!id) throw new GuilderiajsError(ErrorCodes.GuildChannelResolve);
        return this._fetchChannelMany(id, options.cache);
      }

      if ('cache' in options) return this._fetchMany(options.cache);
      throw new GuilderiajsError(ErrorCodes.InviteResolveCode);
    }
  ...
  async _fetchSingle({ code, cache, force = false }) {
    ...
    if (!invite) throw new GuilderiajsError(ErrorCodes.InviteNotFound);
    return invite;
  }
