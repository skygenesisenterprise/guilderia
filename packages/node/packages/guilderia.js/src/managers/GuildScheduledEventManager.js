...
const { GuildScheduledEventEntityType, Routes } = require('guilderia-api-types/v10');
...
class GuildScheduledEventManager extends CachedManager {
  ...
  async create(options) {
    ...
    if (entityType === GuildScheduledEventEntityType.External) {
      ...
      if (!channel_id) throw new GuilderiajsError(ErrorCodes.GuildVoiceChannelResolve);
  ...
  /**
   * Obtains one or more guild scheduled events from Guilderia, or the guild cache if it's already available.
   *
  ...
  async edit(guildScheduledEvent, options) {
    ...
    if (!guildScheduledEventId) throw new GuilderiajsError(ErrorCodes.GuildScheduledEventResolve);
