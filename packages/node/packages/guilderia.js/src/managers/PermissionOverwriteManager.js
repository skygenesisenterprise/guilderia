...
const { OverwriteType, Routes } = require('guilderia-api-types/v10');
...
class PermissionOverwriteManager extends CachedManager {
  ...
  async upsert(userOrRole, options, { reason, type } = {}, existing = undefined) {
    ...
    if (typeof resolvedType !== 'number') {
      ...
      if (!resolvedUserOrRole) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'parameter', 'User nor a Role');
  ...
  async delete(userOrRole, reason) {
    const userOrRoleId = this.channel.guild.roles.resolveId(userOrRole) ?? this.client.users.resolveId(userOrRole);
    if (!userOrRoleId) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'parameter', 'User nor a Role');
