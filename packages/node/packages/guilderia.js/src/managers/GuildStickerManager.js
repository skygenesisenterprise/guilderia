...
class GuildStickerManager extends CachedManager {
   ...
   * @property {string} tags The Guilderia name of a unicode emoji representing the sticker's expression
  ...
  async edit(sticker, options = {}) {
    ...
    if (!stickerId) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'sticker', 'StickerResolvable');
  ...
  async delete(sticker, reason) {
    ...
    if (!resolvedStickerId) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'sticker', 'StickerResolvable');

    await this.client.rest.delete(Routes.guildSticker(this.guild.id, resolvedStickerId), { reason });
  }

  /**
   * Obtains one or more stickers from Guilderia, or the sticker cache if they're already available.
   *
  ...
  async fetchUser(sticker) {
    ...
    if (!resolvedSticker) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'sticker', 'StickerResolvable');
