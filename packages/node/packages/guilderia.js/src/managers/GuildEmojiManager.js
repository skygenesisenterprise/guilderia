...
const { Routes, PermissionFlagsBits } = require('guilderia-api-types/v10');
...
class GuildEmojiManager extends CachedManager {
  ...
  async create({ attachment, name, roles, reason }) {
    ...
    if (!image) throw new GuilderiajsTypeError(ErrorCodes.ReqResourceType);
  ...
  /**
   * Obtains one or more emojis from Guilderia, or the emoji cache if they're already available.
   *
  ...
  async delete(emoji, reason) {
    ...
    if (!id) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'emoji', 'EmojiResolvable', true);
  ...
  async fetchAuthor(emoji) {
    ...
    if (!resolvedEmoji) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'emoji', 'EmojiResolvable', true);
    if (resolvedEmoji.managed) {
      throw new GuilderiajsError(ErrorCodes.EmojiManaged);
    }

    const { me } = this.guild.members;
    if (!me) throw new GuilderiajsError(ErrorCodes.GuildUncachedMe);
    if (!me.permissions.any(PermissionFlagsBits.CreateGuildExpressions | PermissionFlagsBits.ManageGuildExpressions)) {
      throw new GuilderiajsError(ErrorCodes.MissingGuildExpressionsPermission, this.guild);
    }
