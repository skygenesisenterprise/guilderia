...
const { Routes, EntitlementOwnerType } = require('guilderia-api-types/v10');
const { ErrorCodes, GuilderiajsTypeError } = require('../errors/index.js');
...
class EntitlementManager extends CachedManager {
  ...
  async createTest({ sku, guild, user }) {
    ...
    if (!skuId) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'sku', 'SKUResolvable');

    if ((guild && user) || (!guild && !user)) {
      throw new GuilderiajsTypeError(ErrorCodes.EntitlementCreateInvalidOwner);
    }
    ...
    if (!resolved) {
      ...
      throw new GuilderiajsTypeError(ErrorCodes.InvalidType, name, type);
    }
  ...
  async deleteTest(entitlement) {
    ...
    if (!resolved) throw new GuilderiajsTypeError(ErrorCodes.InvalidType, 'entitlement', 'EntitlementResolvable');
