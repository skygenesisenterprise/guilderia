 ...
 * - A URL <warn>When provided a URL, guilderia.js will fetch the URL internally in order to create a Buffer.
...
async function resolveFile(resource) {
  ...
  if (typeof resource === 'string') {
    ...
    if (!stats.isFile()) throw new GuilderiajsError(ErrorCodes.FileNotFound, file);
    return { data: await fs.readFile(file) };
  }

  throw new GuilderiajsTypeError(ErrorCodes.ReqResourceType);
}
