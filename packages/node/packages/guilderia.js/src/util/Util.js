...
const { lazy } = require('@guilderiajs/util');
const { ChannelType, RouteBases, Routes } = require('guilderia-api-types/v10');
const { fetch } = require('undici');
const { Colors } = require('./Colors.js');
// eslint-disable-next-line import-x/order
const { GuilderiajsError, GuilderiajsRangeError, GuilderiajsTypeError, ErrorCodes } = require('../errors/index.js');
...
/**
 * Gets the recommended shard count from Guilderia.
 *
 * @param {string} token Guilderia auth token
...
async function fetchRecommendedShardCount(token, { guildsPerShard = 1_000, multipleOf = 1 } = {}) {
  if (!token) throw new GuilderiajsError(ErrorCodes.TokenMissing);
  const response = await fetch(RouteBases.api + Routes.gatewayBot(), {
    method: 'GET',
    headers: { Authorization: `Bot ${token.replace(/^bot\s*/i, '')}` },
  });
  if (!response.ok) {
    if (response.status === 401) throw new GuilderiajsError(ErrorCodes.TokenInvalid);
    throw response;
  }
 ...
 * - A Guilderia custom emoji (`<:name:id>` or `<a:name:id>`)
 *
...
function resolveColor(color) {
  ...
  if (!Number.isInteger(resolvedColor)) {
    throw new GuilderiajsTypeError(ErrorCodes.ColorConvert, color);
  }

  if (resolvedColor < 0 || resolvedColor > 0xffffff) {
    throw new GuilderiajsRangeError(ErrorCodes.ColorRange);
  }
...
/**
 * Sorts by Guilderia's position and id.
 *
 * @param {Collection} collection Collection of objects to sort
 * @returns {Collection}
 */
function guilderiaSort(collection) {
...
function parseWebhookURL(url) {
  const matches =
    /https?:\/\/(?:ptb\.|canary\.)?guilderia\.com\/api(?:\/v\d{1,2})?\/webhooks\/(?<id>\d{17,19})\/(?<token>[\w-]{68})/i.exec(
      url,
    );
...
exports.guilderiaSort = guilderiaSort;
